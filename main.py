...
import json
import ultra
from ultra.utils.sys_tools import init_seed, get_local_time, write_summary

# rank list size should be read from data
parser = argparse.ArgumentParser(description='Pipeline commandline argument')
parser.add_argument("--data_dir", type=str, default="./Yahoo_letor_weak/tmp_data/",
...
parser.add_argument("--model_dir", type=str, default="./tests/pbm_weak/Yahoo/",
                    help="The directory for model and intermediate outputs.")
parser.add_argument("--output_dir", type=str, default="./tests/pbm_weak/Yahoo/",
...
# model
parser.add_argument("--setting_file", type=str, default="./example/offline_setting/upe_exp_settings.json",
...
def train(exp_settings):
    ...
    modelLogName = '/{}_{}'.format(exp_settings['learning_algorithm'], get_local_time())
    train_writer = torch.utils.tensorboard.SummaryWriter(log_dir=args.model_dir + '/train_log' + modelLogName)
    valid_writer = torch.utils.tensorboard.SummaryWriter(log_dir=args.model_dir + '/valid_log' + modelLogName)
    test_writer = None
    if args.test_while_train:
        test_writer = torch.utils.tensorboard.SummaryWriter(log_dir=args.model_dir + '/test_log' + modelLogName)
    ...
    while True:
        ...
        write_summary(train_writer, "Train Summary", summary, model.global_step, key_name="RawIPW")
        write_summary(train_writer, "Train Summary", summary, model.global_step, key_name="NormIPW")
        write_summary(train_writer, "Train Summary", summary, model.global_step, key_name="NormIRW")
        write_summary(train_writer, "Train Summary", summary, model.global_step, key_name="pretrain")
        write_summary(train_writer, "Train Summary", summary, model.global_step, key_name="grad_norm")
        # train_writer.add_scalars("Train Summary", summary, model.global_step)
